...
```
elapsed   = max(0, floor((nowMs - phaseStartMs_) / 1000))   // 墙钟回拨时钳制为 0
...
`start(..., leanFirstSecond)`：仅**播放/继续**（非重置时长）时 `phaseStartMs_ = nowMs - 1000`，首屏即少 1 秒有反馈；**Next、±60s** 用 `nowMs`，避免连点每次多减 1 秒。
...
3. `clock_start(WorkTime)` → `start(WorkTime, counter)`：
   ...
   - 续计时 `leanFirstSecond=true`（起点 `now-1s`）；Next 重置为 `leanFirstSecond=false`
   - `ensure_work_tick_running()`：立即 `on_tick()` 一次，再启 1s `QTimer`（`Qt::PreciseTimer`）
...
- **正在跑**（`get_running()`）→ 暂停
- **否则**（含休眠冻结后 `running_=false`）→ 继续 / 续计

> 勿用 `workStep_ == WorkTime` 判断暂停：休眠冻结后 `workStep_` 仍为 `WorkTime` 但 `running_=false`，旧逻辑会误点成「再暂停」。

设置页与托盘经 `dispatch_user_action` → `run_command(ActionPlayPause)`。
...
| 场景 | 触发 | 行为 |
| --- | --- | --- |
| 主浮窗工作中 | 1s `TickTimer` | `detect_sleep_gap()` + `sync_from_wall_clock()` |
| 主浮窗休眠冻结后唤醒 | `ApplicationActive` / `WM_POWERBROADCAST` / 唤醒看门狗 | `try_resume_after_wake()` |
| 主浮窗运行中墙钟异常 | 每次 `sync` | `realign_phase_start_if_ahead()`（`now < phaseStartMs_` 时对齐） |
...
工作时休眠/挂起规则详见 [计时与休眠.md](./计时与休眠.md)。`sync_from_wall_clock()` 在剩余 ≤ 0 时会 emit `clock_out`，从而进入下一阶段。
